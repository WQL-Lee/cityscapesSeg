...
def convert_coco_json(jsondir, datasetdir,savedir,use_segments=False, cls91to80=False):
    save_dir = make_dirs(savedir)  # output directory
    coco80 = coco91_to_coco80_class()

    # Import json
    for json_file in sorted(Path(jsondir).resolve().glob('*.json')):
     
        fn_img= Path(save_dir) / 'images' /json_file.stem.replace('instancesonly_filtered_gtFine_', '')
        fn_img.mkdir()
        
        
        
        fn_lb = Path(save_dir) / 'labels' / json_file.stem.replace('instancesonly_filtered_gtFine_', '')  # folder name
        fn_lb.mkdir()
        ...
        # copy dataset'images to processed directory   
        for img in data["images"]:
            img_path = os.path.join(datasetdir,img["file_name"])
            # copy images
            shutil.copy(img_path,fn_img)
        ...
        for img_id, anns in tqdm(imgToAnns.items(), desc=f'Annotations {json_file}'):
            ...
            # subdir=os.path.join(fn_lb,file_dir)
            ...
            # Write
            with open(os.path.join(fn_lb,outfilename), 'a') as file:
...
if __name__ == '__main__':
    ...
    convert_coco_json('./dataset/cityscapes/annotations',  # directory with *.json
                        use_segments=True,
                        cls91to80=False)
